...
@pytest.fixture
def akismet_sync_class(
    request: pytest.FixtureRequest,
) -> typing.Type[akismet.TestSyncClient]:
    """
    Build and return a (sync) Akismet test client class.

    """
    marker = request.node.get_closest_marker("akismet_sync_client")
    if marker is None:
        _comment_check_response = akismet.CheckResponse.SPAM
        _verify_key_response = True
    else:
        _comment_check_response = marker.kwargs.get(
            "comment_check_response", akismet.CheckResponse.SPAM
        )
        _verify_key_response = marker.kwargs.get("verify_key_response", True)

    class _SyncClient(akismet.TestSyncClient):
        """
        Test Akismet client.
...
@pytest.fixture
def akismet_async_class(
    request: pytest.FixtureRequest,
) -> typing.Type[akismet.TestAsyncClient]:
    """
    Build and return an (async) Akismet test client class.

    """
    marker = request.node.get_closest_marker("akismet_async_client")
    if marker is None:
        _comment_check_response = akismet.CheckResponse.SPAM
        _verify_key_response = True
    else:
    ...
    class _AsyncClient(akismet.TestAsyncClient):
        """
        Test Akismet client.
...
@pytest.fixture
def akismet_sync_client(
    akismet_sync_class: type[akismet.SyncClient],
) -> akismet.SyncClient:
    """
    Return a (sync) Akismet test client.
...
@pytest.fixture
def akismet_ham_role() -> str:
    """
    Return a role value which Akismet always considers to be non-spam.
...
@pytest.fixture
def akismet_spam_author() -> str:
    """
    Return an author value which Akismet always considers to be spam.
