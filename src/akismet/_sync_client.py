...
class SyncClient:
    ...
    * :meth:`verify_key`
    ...
    def __init__(
        self, config: _common.Config, http_client: typing.Optional[httpx.Client] = None
    ) -> None:
        self._http_client = http_client or _common._get_sync_http_client()
        self._config = config

    @classmethod
    def client(cls, http_client: typing.Optional[httpx.Client] = None) -> "SyncClient":
        """
        Constructor of :class:`SyncClient`.

        This is always preferred over the default ``SyncClient()`` constructor, because
